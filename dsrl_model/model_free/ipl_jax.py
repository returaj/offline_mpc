...
def main(args, cfg_env=None):
    ...
    while steps < config["total_iteration"]:
        ...
        if (steps % config["log_freq"] == 0) and (not logger.logged):
            ...
            logger.log_tabular("Loss/Loss_value_pos_loss", value_pos_loss.item())
            logger.log_tabular("Loss/Loss_value_neg_loss", value_neg_loss.item())
            logger.log_tabular("Loss/Loss_value_union_loss", value_union_loss.item())

            logger.log_tabular("Loss/Loss_critic", critic_loss.item())
            logger.log_tabular(
                "Loss/Loss_critic_pos_neg_loss", critic_pos_neg_loss.item()
            )
            logger.log_tabular(
                "Loss/Loss_critic_pos_union_loss", critic_pos_union_loss.item()
            )
            logger.log_tabular(
                "Loss/Loss_critic_union_neg_loss", critic_union_neg_loss.item()
            )
