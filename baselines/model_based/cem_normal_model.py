...
from baselines.utils.models import VCritic, GaussianMLP
...
def get_horizon_cost(dynamics, critic, init_state, controls, noises):
    ...
    with torch.no_grad():
        for act, n in zip(controls, noises):
            mean, std = dynamics(torch.cat([state, act], dim=1))
            delta = mean + n * std
            state = (state + delta) / 2.0
...
def cem_policy(dynamics, critic, eval_obs, act, config, device):
    ...
    for _ in range(config["max_iter"]):
        ...
        noises = torch.randn(
            size=(num_samples, horizon, obs_size), dtype=torch.float32, device=device
        )
        costs = torch.vmap(cost_fn, in_dims=(0, 0))(samples, noises)
...
def main(args, cfg_env=None):
    ...
    labels = np.concatenate(
        [
            np.ones(neg_observations.shape[0]),
            np.zeros(union_observations.shape[0]),
        ],
    )
    ...
    for epoch in range(num_epochs):
        ...
        for target_obs, target_act, target_next_obs, label in dataloader:
            ...
            pred_mean, pred_std = dynamics(torch.cat([target_obs, target_act], dim=1))
            noise = torch.randn(
                size=pred_mean.shape, dtype=torch.float32, device=device
            )
            pred_delta = pred_mean + noise * pred_std
            pred_next_obs = (target_obs + pred_delta) / 2.0
            ...
            logger.store(
                **{
                    ...
                    "Loss/Loss_dynamics_kl": dynamics_kl_loss.mean().item(),
